...
#         img=cv2.imread(img_path)
    ...
    def save_feature_to_img(self, img_path, aug_feature=None,  save_image=False):
        #to numpy
        feature = self.get_single_feature(img_path)
        feature = feature.data.numpy()
        flat_feature = feature.flatten()
        if aug_feature is not None:
            flat_feature = np.concatenate((flat_feature, aug_feature))
        # use sigmod to [0,1]
        feature = np.array([self.sigmoid(value) for value in flat_feature])
        # feature = 1.0 / (1 + np.exp(-1 * feature))
        # to [0,255]
        feature = np.round(feature * 255)
        if save_image:
            # feature.reshape(shape=shape)
