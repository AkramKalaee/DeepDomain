...
from tools.DEEPDOMAIN.pathway_grad.cdrp import main as cdrp
from tools.DEEPDOMAIN.pathway_grad.cdrp import main2 as r_cdrp
from tools.DEEPDOMAIN.pathway_grad.src import Plot_tools
...
class Pruner:
    def __init__(self, model, input, device, label = None, output_orig=None):
        ...
        if label and output_orig is not None:
            ...
            self.output_orig = self.model.predict_by_image_data(input, device).detach()
    ...
    def set_dropout(self, dropout_value=0.0):
        for name, module in self.model.named_modules():
            if isinstance(module, nn.Dropout) or isinstance(module, nn.LSTM):
                module.dropout = dropout_value
    ...
    def _forward(self, input):
        ...
        output = self.model.predict_by_image_data(input=input, device=self.device)
        return output
    ...
    def prune_dgr(self, percentile_to_prune, r=False, debug=False):
        ...
        _ = self.model.predict_by_image_data(input=self.input, device=self.device)
        acts = []
        for a in self.activations:
