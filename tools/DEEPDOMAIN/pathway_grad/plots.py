...
def make_confusion_matrix(m, s):
    ...
    plt.show()
    plt.savefig(f'{base_dir}/matrix_sparsity_{s}.png')
    plt.close()
...
def make_deads_barplot(a, s):
    ...
    # plt.show()
    plt.savefig(f'{base_dir}/deads_barplot_sparsity_{s}.png')
    plt.close()
...
def layerwise(dirs, s):
    ...
    plt.savefig(f'{base_dir}/layerwise_jaccard.png')
...
def layerwise_all(d, s):
    ...
    #plt.show()
    #plt.show()
    plt.savefig(f'{base_dir}/layerwise_all.png')
    plt.close()
...
def layerwise_dead(d, s):
    ...
    plt.tight_layout(0.05, 0.01)
    plt.show()
    plt.savefig(f'{base_dir}/layerwise_deads.png')
    plt.close()

base_dir = f"{SRC_DIR}/tools/DEEPDOMAIN/pathway_grad/path_decoding/maximizing_img"

layerwise_all(f'{base_dir}/paths_logs_80/', 80)
# layerwise_all('./paths_logs_90/', 90)
# layerwise_all('./paths_logs_99/', 99)

make_confusion_matrix(load_mean_paths(f'{base_dir}/paths_logs_80/jaccardsfinal.npy'), 80)
...
make_deads_barplot(np.load(f'{base_dir}/paths_logs_80/deadsfinal.npy'), 80)
