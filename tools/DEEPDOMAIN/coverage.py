...
class Coverage:
    ...
    def assess(self, data_loader):
        ...
        for data, *_ in data_loader:
            # print(f"counter: {counter},    length: {len(data)}")
...
class SurpriseCoverage(Coverage):
    def init_variable(self, hyper, min_var, num_class, log_path=None):
    ...
    def build_SA(self, data_batch, label_batch):
        ...
        SA_batch = SA_batch[~torch.any(torch.isnan(SA_batch), dim=1)]
        SA_batch = SA_batch[~torch.any(torch.isinf(SA_batch), dim=1)]
...
class NLC(Coverage):
    def init_variable(self, hyper=None, log_path=None):
...
class NC(Coverage):
    def init_variable(self, hyper, log_path=None):
        assert hyper is not None
        self.threshold = hyper
        self.coverage_dict = {}
        for (layer_name, layer_size) in self.layer_size_dict.items():
    ...
    def calculate(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            cove_dict[layer_name] = is_covered | self.coverage_dict[layer_name].type(torch.ByteTensor).to(self.device).bool()
        return cove_dict
...
class KMNC(Coverage):
    ...
    def set_range(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            self.range_dict[layer_name][0] = is_less.type(torch.FloatTensor).to(self.device) * cur_min + (
                ~is_less).type(torch.FloatTensor).to(self.device) * self.range_dict[layer_name][0]
            self.range_dict[layer_name][1] = is_greater.type(torch.FloatTensor).to(self.device) * cur_max + (
                ~is_greater).type(torch.FloatTensor).to(self.device) * self.range_dict[layer_name][1]
    ...
    def calculate(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            div = (~div_index).type(torch.FloatTensor).to(self.device) * 1e-6 + div_index.type(torch.FloatTensor).to(
                self.device) * (u_bound - l_bound)
            multisec_output = torch.ceil((layer_output - l_bound) / div * self.k).type(torch.LongTensor).to(
                self.device) * multisec_index.type(torch.LongTensor).to(self.device)
            ...
            multisec_cove_dict[layer_name] = multisec_covered.type(torch.ByteTensor).to(self.device).bool() | (
            self.coverage_dict['multisec'][layer_name]).type(torch.ByteTensor).to(self.device).bool()
    ...
    def coverage(self, cove_dict):
        ...
        multisec_rate = multisec_cove.item() / multisec_total
        return multisec_rate
...
class SNAC(KMNC):
    ...
    def calculate(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            upper_cove_dict[layer_name] = upper_covered | (self.coverage_dict['upper'][layer_name]).type(
                torch.ByteTensor).to(self.device).bool()
    ...
    def coverage(self, cove_dict):
        ...
        upper_rate = upper_cove.item() / upper_total
        return upper_rate
...
class NBC(KMNC):
    def init_variable(self, hyper, log_path=None):
        ...
        for (layer_name, layer_size) in self.layer_size_dict.items():
            # num_neuron = layer_size[0]
            num_neuron = np.prod(np.array(layer_size))
    ...
    def calculate(self, data):
        ...
        layer_output_dict = tool.get_layer_output(self.model, data)
        i = 0
        n_critical_layer = len(self.critical_layer_paths)
        for (layer_name, _layer_output) in layer_output_dict.items():
            if i < n_critical_layer and self.critical_layer_paths[i].shape[0] == _layer_output.shape[0]:
                n_neurons = torch.LongTensor(np.array(np.where(self.critical_layer_paths[i]) != 0)).flatten().to(self.device)
                layer_output = torch.index_select(_layer_output, 0, n_neurons)
                l_bound = torch.index_select(self.range_dict[layer_name][0], 0, n_neurons)
                u_bound = torch.index_select(self.range_dict[layer_name][1], 0, n_neurons)

                _coverage_dict_lower = torch.index_select(self.coverage_dict['lower'][layer_name], 0, n_neurons)
                _coverage_dict_upper = torch.index_select(self.coverage_dict['upper'][layer_name], 0, n_neurons)
                i += 1
            else:
                layer_output = _layer_output
                [l_bound, u_bound] = self.range_dict[layer_name]
                _coverage_dict_lower = self.coverage_dict['lower'][layer_name]
                _coverage_dict_upper = self.coverage_dict['upper'][layer_name]
                # continue

            lower_covered = (layer_output < l_bound).sum(0) > 0
            upper_covered = (layer_output > u_bound).sum(0) > 0

            lower_cove_dict[layer_name] = lower_covered | (_coverage_dict_lower).type(
                torch.ByteTensor).to(self.device).bool()
            upper_cove_dict[layer_name] = upper_covered | (_coverage_dict_upper).type(
                torch.ByteTensor).to(self.device).bool()
    ...
    def calculate_v2(self, data):
        ...
        for (layer_name, _layer_output) in layer_output_dict.items():
            if i < n_critical_layer and self.critical_layer_paths[i].shape[0] == _layer_output.shape[0]:
                n_neurons = torch.LongTensor(np.where(self.critical_layer_paths[i] != 0)).flatten().to(self.device)
                layer_output = torch.index_select(_layer_output, 0, n_neurons)
                l_bound = torch.index_select(self.range_dict[layer_name][0], 0, n_neurons)
                u_bound = torch.index_select(self.range_dict[layer_name][1], 0, n_neurons)
    ...
    def calculate_v1(self, data):
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            lower_cove_dict[layer_name] = lower_covered | (self.coverage_dict['lower'][layer_name]).type(
                torch.ByteTensor).to(self.device).bool()
            upper_cove_dict[layer_name] = upper_covered | (self.coverage_dict['upper'][layer_name]).type(
                torch.ByteTensor).to(self.device).bool()
    ...
    def coverage(self, cove_dict):
        ...
        for layer_name in lower_cove_dict.keys():
            ...
            upper_total += len(upper_covered)
        lower_rate = lower_cove.item() / lower_total
        upper_rate = upper_cove.item() / upper_total
...
class CC(Coverage):
    ...
    def init_variable(self, hyper, log_path):
        assert hyper is not None
        self.threshold = hyper
    ...
    def init_log(self):
        with open(self.log_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["nearest_distance"])
        ...
        for (layer_name, layer_output) in layer_output_dict.items():
            ...
            for single_output in layer_output:
                ...
                if len(self.distant_dict[layer_name]) > 0:
                    ...
                    self.log({"nearest_distance": nearest_distance})
