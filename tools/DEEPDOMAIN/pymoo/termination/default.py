...
class DefaultTermination(Termination):
    ...
    def __init__(self, x, cv, f, n_max_gen=1000, n_max_evals=100000, max_time="01:00:00") -> None:
        super().__init__()
        self.x = x
        self.cv = cv
        self.f = f
        ...
        self.criteria = [self.x, self.cv, self.f, self.max_gen, self.max_evals, self.max_time]

    def _update(self, algorithm):
        p = []
        for criterion in self.criteria:
            p.append(criterion.update(algorithm))
