...
class DesignSpaceTermination(DeltaToleranceTermination):
    ...
    def _delta(self, prev, current):
        ...
        if prev.dtype == float and current.dtype == float:
            ...
            return Hypervolume(current).do(prev)
        else:
            _prev = np.array([x.values() for x in prev]).reshape(-1,1)
            _current = np.array([x.values() for x in current]).reshape(-1,1)

            return np.mean([np.sum(e != _prev, axis=1).max() / len(e) for e in _current])
