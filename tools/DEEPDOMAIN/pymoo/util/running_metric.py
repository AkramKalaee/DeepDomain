...
class RunningMetricAnimation(AnimationCallback):
    ...
    def __init__(self,
        ...
        self.running = RunningMetric(indicator=indicator)
    ...
    def do(self, _, algorithm, force_plot=False, **kwargs):
        ...
        if (tau > 0 and tau % self.delta_gen == 0) or force_plot:
            ...
            if self.key_press:
                ...
                plt.draw()
                # fig.show()
                file_path = f"{self.save_path}/metric_generation_{algorithm.n_gen}.jpg"
                plt.savefig(file_path)
                # plt.waitforbuttonpress()
                # cv2.waitKey(1)
                plt.close()

                with open(f"{self.save_path}/metric_generation.txt", "w") as file:
                    file.write(str(f))
                    file.close()

            lhs = max(max(running.delta_ideal), max(running.delta_nadir), max(running.delta_f))

            if lhs <= self.tol:
                print(f"--termination condition: {lhs} <= {self.tol}")
                algorithm.termination.force_termination = True
