...
class UdacityDataset(Dataset):
    def __init__(self, csv_file, root_dir, transform=None, select_camera=None, slice_frames=None, select_ratio=1.0, select_range=None, optical_flow=True, seq_len=0, img_size=(224, 224)):
        ...
        if select_camera:
            assert select_camera in ['left', 'right', 'center'], "Invalid camera: {}".format(select_camera)
            # camera_csv = camera_csv[camera_csv['frame_id']==select_camera]
            camera_csv = camera_csv[camera_csv['frame_id'].str.startswith(select_camera)]
    ...
    def read_data_single(self, idx, augs):
        path = os.path.join(self.root_dir, self.camera_csv['frame_id'].iloc[idx])
        path = f"{path}.jpg"
        ...
        if self.optical_flow:
            if idx != 0:
                path = os.path.join(self.root_dir, self.camera_csv['frame_id'].iloc[idx - 1])
